...
class Solution {
  public:
    ...
    int treePathsSumUtil(Node* node, int currentSum) {
        ...
        // Otherwise, recursively sum up for left and right subtrees
        int leftSum = treePathsSumUtil(node->left, currentSum);
        int rightSum = treePathsSumUtil(node->right, currentSum);

        // Return the total sum of left and right subtree paths
        return leftSum + rightSum;
    }

    int treePathsSum(Node* root) {
        // Start the recursive function with initial sum as 0
