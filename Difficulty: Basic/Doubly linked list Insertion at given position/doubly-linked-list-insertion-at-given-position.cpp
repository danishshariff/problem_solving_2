...
class Solution {
  public:
    ...
    Node *addNode(Node *head, int pos, int data) {
        ...
        // Traverse the list until reaching the required position
        while (cnt < pos && temp != NULL) {
            temp = temp->next;
            cnt++;
        }
        
        // If the list is empty or temp is null, return head
        if (temp == NULL) {
            return head;
        }
        
        // Insert the new node after the current node 'temp'
        ...
        // If nextNode is not null, update its previous pointer
        if (nextNode != NULL) {
